// Income threshold proof circuit with commitment - V2 with explicit wires
...
template IncomeThreshold() {
    ...
    // EXPLICIT intermediate wires to force public inputs to be independent
    signal thresholdWire;
    signal commitmentWire;
    
    thresholdWire <== threshold;
    commitmentWire <== commitment;
    
    // Step 1: Verify income >= threshold
    component geq = GreaterEqThan(32);
    geq.in[0] <== income;
    geq.in[1] <== thresholdWire;
